...
# ─────────────────────────────────────────────────────────────────────────────
# StatBunker uses a specific canonical name for each team that often differs
# from the version we receive from API-Sports or the user's match feed.
#
# Example mismatches encountered live:
#   - "Inter"            → StatBunker has "Internazionale"
#   - "PSG"              → StatBunker has "Paris Saint Germain"
#   - "Atletico"         → "Atletico Madrid"
#   - "Spain"            → can be "España"
#
# `TEAM_ALIASES` maps every known alias (normalised) to the **canonical**
# StatBunker name (also normalised). `_best_match` resolves the input
# through this dict BEFORE falling back to fuzzy matching, eliminating
# almost all "team_not_in_table" misses for Tier 1/2 clubs and the major
# national teams.

# Each entry maps alias → canonical StatBunker key (already normalised).
...
def _resolve_alias(name: str) -> str:
    """Map a team-name input through `TEAM_ALIASES`.

    Idempotent: if the normalised name is already canonical it is
    returned unchanged. Otherwise the alias's target replaces it.
    """
